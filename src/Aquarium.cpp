...
void FlashFish::move() {
    //Flash Fish might move more fast
    m_x += m_dx * (m_speed * 3.0);
    m_y += m_dy * (m_speed * 3.0);
    if(m_dx < 0){
        this->m_sprite->setFlipped(true);
    }else{
        this->m_sprite->setFlipped(false);
...
//Refactorizacion de Repopulate, ahora los levels heredan de AquariumLevel
vector<AquariumCreatureType> AquariumLevel::Repopulate(){
    vector<AquariumCreatureType> toRepopulate;

    for(shared_ptr<AquariumLevelPopulationNode> node : this->m_levelPopulation){
